...
Original file is located at
    https://colab.research.google.com/drive/1HIdmqhsD-CG6ERYm4Ahba9-1uLxCbMic
"""
...
def inverse_mean_scale_normalize_cluster(array, normalization_factor):
    ...
    print('inverse_mean_scale_normalize_cluster:', len(normalization_factor))
    return np.array(array) / normalization_factor
...
def inverse_normalize_0_1_cluster(array_true, array_pred):
    ...
    for index in range(cols):
        ...
        print('--------------------|||||||||||||||||||------------------')
        print('array_pred_rescaled[:, index]:', array_pred_rescaled[:, index])
        print('array_true[:, index]:', array_true[:, index])
        print('--------------------|||||||||||||||||||------------------')
...
def get_callbacks(patient):
    ...
    callbacks = [callback_checkpoint, callback_tensorboard, callback_reduce_lr, callback_early_stopping]
...
def get_prediction_clustered(X, model, normalization_factor=[], start=0, length=100):
    ...
    print('--------------------------------------')
#     Y_pred_rescaled = inverse_normalize_0_1_cluster(Y_AdjShip, Y_pred[0])
...
def predict_test(patient, pt_dict):
    #Hyperparameters
    hyperparameters = {
        'cell_dimension': 30,
        'batch_size': 128,
    ...
    df = loadDataframe(patient, directory='test_data/test_with_missing')
    df_cols = df[cols]
#     print('df_cols:', df_cols.head())
#     print('df_cols.shape:', df_cols.shape)
...
#     print('df_normalize:', df_normalize.head())
#     print('df_normalize.shape:', df_normalize.shape)
    
    test_size = len(df.index)
    
    X_test = Y_test = df_normalize.values
#     print('X_train.shape', X_test.shape)
#     print('Y_train.shape', Y_test.shape)
    
    X_test, Y_test = moving_window_data_formatter_train(X_test, Y_test, test_size, hyperparameters)
#     print('X_train.shape', X_test.shape)
#     print('Y_train.shape', Y_test.shape)    
    
    X_test = get_keras_shape(X_test,
                              time_steps=hyperparameters['input_sequence_length'], 
                              num_features = len(predictors))
...
#     print("Y_pred.shape: ",Y_pred.shape)
    ...
    unscaled = scaler.inverse_transform(Y_pred)
    df_unscaled = pd.DataFrame(data = unscaled, columns = cols)
    df_first_window = pd.DataFrame(pd.np.zeros((hyperparameters['input_sequence_length'], len(cols))), columns = cols)
    df_unscaled = pd.concat([df_first_window, df_unscaled], axis=0)
#     print("df_unscaled.shape: ",df_unscaled.shape)
#     print("df_cols.shape: ",df_cols.shape)
#     print("df_normalize.shape: ",df_normalize.shape)
    
    key = patient.split('.')[0]
    df_cols = create_test(df_cols, df_normalize, df_unscaled, hyperparameters, pt_dict[key])
    df_cols['time'] = df['time'] 
    df_cols = df_cols.reindex(columns= ['time'] + cols)

                
    saveDataframe(df_cols, name_dir='train_data/processed_many_to_many', name_file =  patient.split('.')[0])

def create_test(df_cols, df_normalize, df_unscaled, hyperparameters, nan_vals):
  points = nan_vals.shape[0]
  for i in range(points):
    row = nan_vals[i][1]
    col = nan_vals[i][0]
    if row < hyperparameters['input_sequence_length']:
      df_cols.iloc[row, col] = df_unscaled.iloc[hyperparameters['input_sequence_length'], col]
    else:
      df_cols.iloc[row, col] = df_unscaled.iloc[row, col]
  return df_cols

import warnings
warnings.filterwarnings('ignore')

getDirectory('test_data')
df = loadDataframe('naidx.csv', directory='test_data')
df['test'] = df['test'].str.replace('X','').astype(int)
df['test'] = df['test'] - 1
df['i'] = df['i'].astype(int) - 1

pt = list(set(df['pt.num']))
pt_dict = {}

for pt in pt:
  pt_df = df.loc[df['pt.num'] == pt]
  del pt_df['pt.num']
  pt_df_values = pt_df.values
  if pt not in pt_dict:
    pt_dict[str(pt)] = pt_df_values
    
getDirectory('train_data/processed_many_to_many')
exists = glob.glob('*')

getDirectory('test_data/test_with_missing')
predictors = targets = cols = ['X' + str(i) for i in range(1, 14)]
patients_test = glob.glob("*")
patients_test = sorted(patients_test)
# print(patients_test)

patients_test = [file for file in patients_test if file not in exists]

for patient in patients_test:
    predict_test(patient, pt_dict)
